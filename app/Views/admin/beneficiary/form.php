    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
            ...
            <input class="appearance-none block w-full bg-gray-100 text-gray-700 border border-gray-300 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-indigo-500 transition-colors"
                ...
                value="<?= old('beneficiary_financier_year') ?? (!isset($beneficiary) ? "":"{$beneficiary['beneficiary_financier_year']}") ?>" required>
        </div>
    </div>
</div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
        ...
        <div>
            ...
            <input class="appearance-none block w-full bg-gray-100 text-gray-700 border border-gray-300 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-indigo-500"
                id="names" name="beneficiary_names" type="text" placeholder="Ingrese nombres" value="<?= old('beneficiary_names') ?? (!isset($beneficiary) ? "":"{$beneficiary['beneficiary_names']}") ?>" required>
        </div>
        ...
        <div>
            ...
            <input class="appearance-none block w-full bg-gray-100 text-gray-700 border border-gray-300 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-indigo-500"
                id="lastnames" name="beneficiary_lastnames" type="text" placeholder="Ingrese apellidos" value="<?= old('beneficiary_lastnames') ?? (!isset($beneficiary) ? "":"{$beneficiary['beneficiary_lastnames']}") ?>" required>
        </div>
        ...
        <div>
            ...
            <input class="appearance-none block w-full bg-gray-100 text-gray-700 border border-gray-300 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-indigo-500"
                id="birthdate" name="beneficiary_birthdate" type="date" value="<?= old('beneficiary_birthdate') ?? (!isset($beneficiary) ? "":"{$beneficiary['beneficiary_birthdate']}") ?>" required>
        </div>
            ...
            <div class="flex gap-2">
                <div class="w-3/4">
                    <input class="appearance-none block w-full bg-gray-100 text-gray-700 border border-gray-300 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-indigo-500"
                        ...
                        value="<?= old('beneficiary_ci') ?? (!isset($beneficiary) ? "":"{$beneficiary['beneficiary_ci']}") ?>" required>
                </div>
                ...
                <div class="w-1/4 relative">
                    <select class="block appearance-none w-full bg-gray-100 border border-gray-300 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-indigo-500"
                        ...
                        <option value="CB" <?= $beneficiary['beneficiary_ci_extension'] == 'CB' ? 'selected' : '' ?>>CB</option>
                        <option value="LP" <?= $beneficiary['beneficiary_ci_extension'] == 'LP' ? 'selected' : '' ?>>LP</option>
                        <option value="SC" <?= $beneficiary['beneficiary_ci_extension'] == 'SC' ? 'selected' : '' ?>>SC</option>
                        <option value="OR" <?= $beneficiary['beneficiary_ci_extension'] == 'OR' ? 'selected' : '' ?>>OR</option>
                        <option value="PT" <?= $beneficiary['beneficiary_ci_extension'] == 'PT' ? 'selected' : '' ?>>PT</option>
                        <option value="TJ" <?= $beneficiary['beneficiary_ci_extension'] == 'TJ' ? 'selected' : '' ?>>TJ</option>
                        <option value="CH" <?= $beneficiary['beneficiary_ci_extension'] == 'CH' ? 'selected' : '' ?>>CH</option>
                        <option value="BE" <?= $beneficiary['beneficiary_ci_extension'] == 'BE' ? 'selected' : '' ?>>BE</option>
                        <option value="PD" <?= $beneficiary['beneficiary_ci_extension'] == 'PD' ? 'selected' : '' ?>>PD</option>
                    </select>
                    <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-700">
                        <svg class="fill-current h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M9.293 12.95l.707.707L15.657 8l-1.414-1.414L10 10.828 5.757 6.586 4.343 8z"/></svg>
                    </div>
        ...
        <div>
            ...
            <select class="block w-full bg-gray-100 border border-gray-300 text-gray-700 py-3 px-4 rounded focus:bg-white focus:border-indigo-500"
                ...
                <option value="Hombre" <?= $beneficiary['beneficiary_gender'] == 'Hombre' ? 'selected' : '' ?>>Hombre</option>
                <option value="Mujer"  <?= $beneficiary['beneficiary_gender'] == 'Mujer' ? 'selected' : '' ?>>Mujer</option>
            </select>
        </div>
        ...
        <div>
            ...
            <input class="appearance-none block w-full bg-gray-100 text-gray-700 border border-gray-300 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-indigo-500"
                id="cellphone" name="beneficiary_cellphone" type="number" placeholder="Ej: 70000000" value="<?= old('beneficiary_cellphone') ?? (!isset($beneficiary) ? "":"{$beneficiary['beneficiary_cellphone']}") ?>" required>
        </div>
        ...
        <div class="md:col-span-2 lg:col-span-1">
            ...
            <input class="appearance-none block w-full bg-gray-100 text-gray-700 border border-gray-300 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-indigo-500"
                id="ref_name" name="beneficiary_ref_name" type="text" placeholder="Nombre del familiar/amigo" value="<?= old('beneficiary_ref_name') ?? (!isset($beneficiary) ? "":"{$beneficiary['beneficiary_ref_name']}") ?>">
        </div>
        ...
        <div>
            ...
            <input class="appearance-none block w-full bg-gray-100 text-gray-700 border border-gray-300 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-indigo-500"
                id="ref_phone" name="beneficiary_ref_phone" type="number" placeholder="Ej: 4444444" value="<?= old('beneficiary_ref_phone') ?? (!isset($beneficiary) ? "":"{$beneficiary['beneficiary_ref_phone']}") ?>">
        </div>

        <?php 
            // 1. Definimos la lista exacta de opciones estándar
            $munis = ['Cochabamba', 'Sacaba', 'Quillacollo', 'Tiquipaya', 'Colcapirua'];
            
            // 2. Obtenemos el valor actual de la BD
            $savedMuni = $beneficiary['beneficiary_municipality'];
            
            // 3. ¿Es personalizado? (Si NO está en la lista y NO está vacío)
        ...
        <div>
            ...
            <div id="wrapper_select_municipality" class="relative <?= $isCustomMuni ? 'hidden' : '' ?>">
                <select class="block appearance-none w-full bg-gray-100 border border-gray-300 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-indigo-500 transition-colors"
                        id="municipality" 
                        /* Si es custom, el select NO lleva name, para no sobrescribir */
                        name="<?= $isCustomMuni ? '' : 'beneficiary_municipality' ?>" 
                        /* Si es custom, el select NO es required */
                    ...
                    <option value="" disabled <?= empty($savedMuni) ? 'selected' : '' ?>>Seleccione...</option>
                    
                    <?php foreach($munis as $muni): ?>
                        <option value="<?= $muni ?>" <?= $savedMuni == $muni ? 'selected' : '' ?>>
                            <?= $muni ?>
                        </option>
                    <?php endforeach; ?>
                    
                    <option value="Otro" <?= $isCustomMuni ? 'selected' : '' ?>>Otro...</option>
                </select>
            ...
            <div id="wrapper_input_municipality" class="flex items-center gap-2 <?= $isCustomMuni ? '' : 'hidden' ?>">
                <div class="relative w-full">
                    <input type="text" 
                        ...
                        name="<?= $isCustomMuni ? 'beneficiary_municipality' : '' ?>" 
                        value="<?= old('beneficiary_municipality') ?? (!isset($beneficiary) ? "":"{$beneficiary['beneficiary_municipality']}") ?>"
                        <?= $isCustomMuni ? 'required' : '' ?>
        ...
        <div class="col-span-full mt-4">
            ...
            <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2">
                Ubicación Exacta <span class="text-red-500">*</span>
            </label>
            
            <div id="map" style="height: 400px; width: 100%; z-index: 0;" class="relative rounded-lg shadow-inner border border-gray-300"></div>
            
            <input type="hidden" id="latitude" name="beneficiary_latitude" 
                value="<?= old('beneficiary_latitude') ?? $beneficiary['beneficiary_latitude'] ?>" required>
            
            <input type="hidden" id="longitude" name="beneficiary_longitude" 
                value="<?= old('beneficiary_longitude') ?? $beneficiary['beneficiary_longitude'] ?>" required>

            <p class="text-xs text-gray-500 mt-1">
                <i class="fas fa-map-marker-alt"></i> 
                <span id="map-status">Mostrando ubicación guardada.</span> (Puedes arrastrar el marcador para corregir).
            </p>
        </div>
...
<div class="mb-8 border-b border-gray-200 pb-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <?php 
            // 1. Lista de opciones estándar (Debe ser idéntica a la del Create)
            $types = ['Gastronomico', 'Textil', 'Servicios', 'Artesano'];
            
            // 2. Valor actual en BD
            $savedType = $beneficiary['beneficiary_entrepreneurship_type'];
            
            // 3. ¿Es personalizado? (No está en la lista y no es vacío)
        ...
        <div>
            ...
            <div id="wrapper_select_entrepreneurship" class="relative <?= $isCustomType ? 'hidden' : '' ?>">
                <select class="block appearance-none w-full bg-gray-100 border border-gray-300 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-indigo-500 transition-colors"
                    ...
                    <option value="" disabled <?= empty($savedType) ? 'selected' : '' ?>>Seleccione una opción</option>
                    ...
                    <option value="Otro" <?= $isCustomType ? 'selected' : '' ?>>Otro...</option>
                </select>
            ...
            <div id="wrapper_input_entrepreneurship" class="flex items-center gap-2 <?= $isCustomType ? '' : 'hidden' ?>">
                <div class="relative w-full">
                    <input type="text" 
                        id="input_entrepreneurship_custom" 
                        name="<?= $isCustomType ? 'beneficiary_entrepreneurship_type' : '' ?>" 
                        value="<?= old('beneficiary_entrepreneurship_type') ?? (!isset($beneficiary) ? "":"{$beneficiary['beneficiary_entrepreneurship_type']}") ?>"
                        <?= $isCustomType ? 'required' : '' ?>
                        class="appearance-none block w-full bg-white text-gray-700 border border-indigo-500 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-indigo-500 shadow-sm"
        ...
        <div>
            ...
            <div id="wrapper_select_sales" class="relative <?= $isCustomChannel ? 'hidden' : '' ?>">
                <select class="block appearance-none w-full bg-gray-100 border border-gray-300 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-indigo-500 transition-colors"
                        id="sales_channel" 
                        name="<?= $isCustomChannel ? '' : 'beneficiary_sales_channel' ?>" 
                        <?= $isCustomChannel ? '' : 'required' ?>
                        onchange="transformToInput(this, 'wrapper_select_sales', 'wrapper_input_sales', 'input_sales_custom'); toggleOnlineChannel(this)">
                    
                    <option value="" disabled <?= empty($savedChannel) ? 'selected' : '' ?>>Seleccione...</option>
                    
                    <?php foreach($channels as $c): ?>
                        <option value="<?= $c ?>" <?= $savedChannel == $c ? 'selected' : '' ?>>
                            <?= $c ?>
                        </option>
                    <?php endforeach; ?>
                    
                    <option value="Otro" <?= $isCustomChannel ? 'selected' : '' ?>>Otro...</option>
                </select>
            ...
            <div id="wrapper_input_sales" class="flex items-center gap-2 <?= $isCustomChannel ? '' : 'hidden' ?>">
                <div class="relative w-full">
                    <input type="text" 
                        id="input_sales_custom" 
                        name="<?= $isCustomChannel ? 'beneficiary_sales_channel' : '' ?>" 
                        value="<?= $isCustomChannel ? esc($savedChannel) : '' ?>"
                        <?= $isCustomChannel ? 'required' : '' ?>
                        class="appearance-none block w-full bg-white text-gray-700 border border-indigo-500 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-indigo-500 shadow-sm"
                        placeholder="Especifique canal...">
                </div>
                
                <button type="button" 
                        onclick="revertToSelect('sales_channel', 'wrapper_select_sales', 'wrapper_input_sales', 'input_sales_custom'); toggleOnlineChannel(document.getElementById('sales_channel'))" 
                        class="bg-red-100 hover:bg-red-200 text-red-600 rounded-full p-2 transition-colors duration-200 focus:outline-none"
                        title="Volver a la lista">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                        <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd" />
        ...
        <div id="online_specify_container" class="hidden md:col-span-2 transition-all duration-300">
            ...
            <input class="appearance-none block w-full bg-gray-100 text-gray-700 border border-gray-300 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-indigo-500"
                id="online_specify" name="beneficiary_online_specify" type="text" placeholder="Ej: Facebook Marketplace, WhatsApp, Página Web..." value="<?= old('beneficiary_online_specify') ?? (!isset($beneficiary) ? "":"{$beneficiary['beneficiary_online_specify']}") ?>">
        </div>
        ...
        <div class="col-span-full">
            ...
            <textarea class="appearance-none block w-full bg-gray-100 text-gray-700 border border-gray-300 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-indigo-500"
                    ...
                    placeholder="Describa brevemente de qué trata su negocio..." required><?= old('beneficiary_entrepreneurship_description') ?? (!isset($beneficiary) ? "":"{$beneficiary['beneficiary_entrepreneurship_description']}") ?></textarea>
        </div>
        ...
        <div class="col-span-full">
            ...
            <input class="appearance-none block w-full bg-gray-100 text-gray-700 border border-gray-300 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-indigo-500"
                ...
                placeholder="Dirección donde realiza la actividad comercial" value="<?= old('beneficiary_sales_address') ?? (!isset($beneficiary) ? "":"{$beneficiary['beneficiary_sales_address']}") ?>" required>
        </div>
...
<div class="mb-8 border-b border-gray-200 pb-6 mt-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="col-span-full">
            ...
            <textarea class="appearance-none block w-full bg-gray-100 text-gray-700 border border-gray-300 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-indigo-500"
                    id="start_motivation" name="beneficiary_start_motivation" rows="2" required><?= old('beneficiary_start_motivation') ?? (!isset($beneficiary) ? "":"{$beneficiary['beneficiary_start_motivation']}") ?></textarea>
        </div>
        ...
        <div id="course_detail_container" class="hidden transition-all duration-300">
            ...
            <input class="appearance-none block w-full bg-gray-100 text-gray-700 border border-gray-300 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-indigo-500"
                id="course_name" name="beneficiary_course_name" type="text" value="<?= old('beneficiary_course_name') ?? (!isset($beneficiary) ? "":"{$beneficiary['beneficiary_course_name']}") ?>">
        </div>
        ...
        <div class="col-span-full">
            ...
            <textarea class="appearance-none block w-full bg-gray-100 text-gray-700 border border-gray-300 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-indigo-500"
                    id="missing_skills" name="beneficiary_missing_skills" rows="2" required><?= old('beneficiary_missing_skills') ?? (!isset($beneficiary) ? "":"{$beneficiary['beneficiary_missing_skills']}") ?></textarea>
        </div>
        ...
        <div>
            ...
            <select class="block w-full bg-gray-100 border border-gray-300 text-gray-700 py-3 px-4 rounded focus:bg-white focus:border-indigo-500" name="beneficiary_business_age" required>
                <option value="" disabled selected>Seleccione...</option>
                <option <?= $beneficiary['beneficiary_business_age'] == 'Menos de 6 meses' ? 'selected':'' ?> value="Menos de 6 meses">Menos de 6 meses</option>
                <option <?= $beneficiary['beneficiary_business_age'] == 'Entre 6 meses y 1 año' ? 'selected':'' ?> value="Entre 6 meses y 1 año">Entre 6 meses y 1 año</option>
                <option <?= $beneficiary['beneficiary_business_age'] == '1 a 3 años' ? 'selected':'' ?> value="1 a 3 años">1 a 3 años</option>
                <option <?= $beneficiary['beneficiary_business_age'] == '' ? 'selected':'' ?> value="Mas de 3 años">Más de 3 años</option>
            </select>
        </div>
        ...
        <div class="col-span-full md:col-span-1">
            ...
            <select class="block w-full bg-gray-100 border border-gray-300 text-gray-700 py-3 px-4 rounded focus:bg-white focus:border-indigo-500"
                ...
                <option value="" disabled selected>Seleccione...</option>
                <option <?= $beneficiary['beneficiary_inst_bought'] == 'Si' ? 'selected':'' ?> value="Si">Sí</option>
                <option <?= $beneficiary['beneficiary_inst_bought'] == 'No' ? 'selected':'' ?> value="No">No</option>
            </select>
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        // 1. Obtener valores guardados o usar defecto (Cochabamba)
        const latInput = document.getElementById('latitude');
        const lngInput = document.getElementById('longitude');
        const statusText = document.getElementById('map-status');

        // Validamos si hay datos guardados (diferentes de 0 o vacío)
        let lat = parseFloat(latInput.value);
        let lng = parseFloat(lngInput.value);
        
        // Coordenadas por defecto (Cochabamba) si no hay datos
        const defaultLat = -17.3938; 
        const defaultLng = -66.1569;

        // Si no hay datos válidos, usamos el default
        if (isNaN(lat) || lat === 0 || isNaN(lng) || lng === 0) {
            lat = defaultLat;
            lng = defaultLng;
            statusText.innerText = "Ubicación por defecto (Mueva el marcador).";
            
            // Opcional: Intentar geolocalizar solo si no había datos previos
            if (navigator.geolocation) {
                navigator.geolocation.getCurrentPosition(function(position) {
                    updatePosition(position.coords.latitude, position.coords.longitude);
                    map.setView([position.coords.latitude, position.coords.longitude], 16);
                    statusText.innerText = "Ubicación actual detectada.";
                });
            }
        }
        ...
        // 3. Crear Marcador (Arrastrable)
        var marker = L.marker([lat, lng], {draggable: true}).addTo(map);

        // --- Eventos ---

        // A) Cuando se arrastra el marcador
        marker.on('dragend', function(e) {
            var position = marker.getLatLng();
            updatePosition(position.lat, position.lng);
        });

        // B) Cuando se hace clic en el mapa
        map.on('click', function(e) {
            updatePosition(e.latlng.lat, e.latlng.lng);
            marker.setLatLng(e.latlng);
        });

        // Función para actualizar los inputs y variables
        function updatePosition(newLat, newLng) {
            latInput.value = newLat;
            lngInput.value = newLng;
            statusText.innerText = "Ubicación actualizada manualmente.";
        }

        // Parche para renderizado correcto si el mapa estaba oculto o en tabs
